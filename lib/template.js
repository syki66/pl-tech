...
module.exports = {
    ...
    noticeList: function (datalist) {
        var list = '<ul><li><div class="date__container">날짜</div><div class="link__container title">제목</div></li>';
        var i = 0;
        while (i < datalist.length) {
            var cdate = util.cdateParser(datalist[i].cdate);
            list = list + `<li>
                                <div class="date__container">${cdate.date} (${cdate.week})</div><div class="link__container"> <a class="board__link" href="/board/${datalist[i].id}">${datalist[i].title}</a></div>
                        </li>`;
            i = i + 1;
        ...
        <body>
            ...
            <div class="container">
                ...
                <div class="inner__container">
                    ...
                    &nbsp;&nbsp&nbsp;&nbsp&nbsp;&nbsp
                    <a href="/board/more/1">공지사항 더보기</a>
                </div>
            </div>            
    ...
    m_board: function (list, ptemplate, isAdminPage) {
        ...
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">`
        template = template + (isAdminPage ? `<title>관리자 설정 페이지 - 공지 관리</title>` : `<title>공지사항 더보기</title>`);
        template = template + `
            <link rel="stylesheet" type="text/css" href="/views/src/styles/page.css" />
            <link rel="stylesheet" href="/views/src/styles/adminPages.css">
            <script src="/views/src/scripts/fetchJson.js"></script>
        ...
        <body>
            <div class="container">
                ...
                <div class="inner__container">`;
        template = template + (isAdminPage ? `<h2>공지사항 글 관리</h2>` : `<h2>공지사항 게시판</h2>`);
        template = template + `${list}`;
        template = template + (isAdminPage ?  `<a class="link__1" href="/admin">돌아가기</a>` : `<a class="link__1" href="/board">돌아가기</a>`);
        template = template + (isAdminPage ?  `<a class="link__2" href="/admin/post">공지사항 작성하기</a>` : ``);
        template = template +`
                </div>
    ...
    m_noticeList: function (datalist, pagenum, isAdminPage) {
        ...
        var list = '<ul><li><div class="board__link title">제목</div></li>';
        var i = 0;
        while (i<datalist.length) {
            list = list + (isAdminPage ? 
                `<li><div class="link__container"><a class="board__link" href="/admin/post/${datalist[i].id}/update?pagenum=${pagenum}">${datalist[i].title}</a></div>
              ...
              `<li><div class="link__container"><a class="board__link" href="/board/${datalist[i].id}?pagenum=${pagenum}">${datalist[i].title}</a></div></li>`);
              i = i + 1;
        // var list = '<ul><li><div class="link__container title">제목</div><div class="del__container">삭제</div></li>';
    ...
    m_edit: function (_id, _title, _contents, page) {
        ...
        <form action ="/admin/post/${_id}/uprocess?_method=patch" method="post"> 
          ...
          <p><input name="title" type="text" value="${_title}"></p>
          <p><textarea name="contents">${_contents}</textarea></p>
          <p><input type="submit" value="수정"></p>
        </form>
        <a href="/admin/post/manage/${page}">돌아가기</a>
        </body>
        </html>
        `
    },
