...
module.exports = {
    ...
    u_noticeList: function (datalist) {
        // /board
        var list = `<ul>
                        <li>
    ...
    c_noticeList: function (datalist, pageNum, isAdminPage) {
        // /admin/post/manage/:pageNum
        var list = '<ul><li><div class="link__container">제목</div></li>';
        var i = 0;
        while (i<datalist.length) {
            let date = `${datalist[i].cdate.substring(5,7) + '/' + datalist[i].cdate.substring(8,10)} (${datalist[i].cdate.substring(11,12)})`;
            list = list + (isAdminPage ? `<li>
                <div class="link__container">
                    ${date}
                    <a class="board__link" href="/admin/notice/${datalist[i].id}/update?pageNum=${pageNum}">${datalist[i].title}</a>
                </div>
                <form class="del__container" action="/admin/notice/${datalist[i].id}/dprocess?_method=DELETE" method="post">
    ...
    a_workerList: function (name, datalist) {
